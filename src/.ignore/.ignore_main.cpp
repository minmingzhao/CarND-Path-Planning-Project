      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            bool check_left_lane = false;
            bool check_right_lane = false;
              ...
              if(d  < (2+4*lane+2) && d > (2+4*lane-2) )
              {
                ...
                if((check_car_s > car_s) && ((check_car_s-car_s)<30))
                {
                  ...
                  if (lane>0)
                  {
                    ...
                    check_left_lane = true;
                  }
                  if (lane<2)
                  {
                    check_right_lane = true;
                  }
                  // if(lane>0)
                  // {lane = 0;}
                }
            ...
            for(int i =0;i<sensor_fusion.size();i++)
            {
              ...
              double delta_vel = check_speed - car_speed;
              if(d < (2+4*lane-2) && d> (2+4*lane-2-4)) // left one lane of my car
              {
                if (delta_s>0)
                {
                  cost = 100/delta_s +(50 - delta_vel)*0.1;
                }
                else
                {
                  cost = -100/delta_s + (50 + delta_vel)*0.1;
              ...
              // double min_s_check = 15;
              // min_s_check = abs(0 - (check_speed - car_speed)*0.44694*(check_car_s>car_s));

              //there is a car at my left lane within 15m ahead or behind, not allow left lane change
              if(d < (2+4*lane-2) && abs(check_car_s-car_s)<min_s_check)
               {
                 left_lane_change = false;
                 if (min_s_check != 15)
                 {cout<< "min_s_check= " << min_s_check << ". (check_speed - car_speed)" << (check_speed - car_speed)*(check_car_s>car_s)<< endl;}
               }
               //there is a car at my right lane within 15min ahead or behind, not allow right lane change
              else if(d > (2+4*lane+2) && abs(check_car_s-car_s)<min_s_check)
              {
